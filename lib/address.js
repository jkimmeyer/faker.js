...
function Address (faker) {
   ...
   * @param {String} format
   */
  this.streetAddress = function (useFullAddress, format) {
    //if format is not specified, use the streetAddress format defined for the locale
    if (typeof useFullAddress === 'undefined') useFullAddress = false;
    if (typeof format === 'undefined'){
      var localeFormat = faker.definitions.address.street_address;
      if (typeof localeFormat === 'string'){
        format = localeFormat;
      } else {
        format = faker.random.arrayElement(localeFormat);
    ...
    return useFullAddress ? (f(format) + " " + faker.address.secondaryAddress()) : f(format);
  }

  /**
   * Returns a random localized street root
   *
   * @method faker.address.streetRoot
   */
  ...
  /**
   * Returns a random localized buildingNumber
   *
   * @method faker.address.buildingNumber
   */
  this.buildingNumber = function () {
    return faker.helpers.replaceSymbols(faker.random.arrayElement(faker.definitions.address.building_number));
  }
